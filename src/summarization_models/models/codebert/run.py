        ...
        for idx, line in enumerate(f):
            ...
            try:
                examples.append(
                    Example(
                        idx=idx,
                        # MODIFIED: Solving surrogate pass issue
                        source=code.encode("utf-8", errors="surrogatepass").decode(
                            "utf-8"
                        ),
                        target=nl.encode("utf-8", errors="surrogatepass").decode(
                            "utf-8"
                        ),
                    )
                )
            except Exception as e:
                print(f"Decode error, skipping example #{idx}")
    return examples
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        features.append(
            InputFeatures(
                example_index, source_ids, target_ids, source_mask, target_mask,
            )
        )
    return features
...
def main():
    ...
    print()
    print(
        f"=> Training with {args.n_gpu} GPUs, torch device count {torch.cuda.device_count()}"
    )
    print()
