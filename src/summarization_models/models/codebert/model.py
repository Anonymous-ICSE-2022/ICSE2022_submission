...
class Seq2Seq(nn.Module):
    ...
    def forward(
        ...
        if target_ids is not None:
            ...
            # Predict
            preds = []
            device = source_ids.get_device()

            zero = torch.zeros(1, device=device, dtype=torch.long)
            for i in range(source_ids.shape[0]):
                context = encoder_output[:, i : i + 1]
                context_mask = source_mask[i : i + 1, :]
                beam = DeviceAgnosticBeam(
                    self.beam_size, self.sos_id, self.eos_id, device=device
                )
                ...
                preds.append(torch.cat(pred, 0).detach().unsqueeze(0))

            preds = torch.cat(preds, 0).cpu()
            return preds


class DeviceAgnosticBeam(object):
