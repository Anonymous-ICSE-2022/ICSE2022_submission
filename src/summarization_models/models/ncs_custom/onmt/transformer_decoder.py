...
from .average_attention import AverageAttention
from .embeddings import Embeddings
from .misc import sequence_mask
from .misc_layers import DecoderBase
from .multihead_attention import MultiHeadedAttention
from .position_ffn import ActivationFunction, PositionwiseFeedForward
...
class TransformerDecoderLayerBase(nn.Module):
    def __init__(
        ...
        dropout,
        attention_dropout,
        self_attn_type: Literal["average", "scaled-dot"] = "scaled-dot",
        ...
        valid_attention_types = ["scaled-dot", "average"]
        assert (
            self_attn_type in valid_attention_types
        ), f"Invalid attention type '{self_attn_type}'. Please choose from the following:\n{valid_attention_types}"
...
class TransformerDecoder(TransformerDecoderBase):
    ...
    def __init__(
        ...
        full_context_alignment: bool = None,
    ...
    def forward(
        self, tgt, memory_bank=None, step=None, **kwargs
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """[summary]

        Parameters
        ----------
        tgt : target long tensor
            Of shape [max_src_len, batch_size, n_features]
        memory_bank : Encoder memory bank, optional
            [description], by default None
        step : int, optional
            step number if in a stepwise calculation, by default None
        memory_lengths: torch.LongTensor,
            of shape [batch_size,]. Specifies lengths of memory bank
        with_align: boolean,
            whether or not to use alignment
        Returns
        -------
        Tuple[torch.Tensor, Dict[str, torch.Tensor]]
            * decoder_outs: of shape [max_target_length, batch_size, target_embedding_size]
            * attention weights
        """
        ...
        output = emb.transpose(0, 1).contiguous()  # batch * len * embedding_dim
        src_memory_bank = memory_bank.transpose(
            0, 1
        ).contiguous()  # batch * max_src * 1
